...
3. Ticket thực thi (task file) nằm trong **`../../al-run/current-task/`** — xem dashboard [`../../al-run/current-task/README.md`](../../al-run/current-task/README.md). Thứ tự demo: **SIM-DEMO-1** infra trước, rồi **SIM-DEMO-2** feature chính (tên file flat gợi ý trong `DEMO-PROJECT-BRIEF.md`; có thể tạo khi cần). Luồng: **`../../al-run/task-lifecycle/`**.
...
| [`../../al-run/current-task/README.md`](../../al-run/current-task/README.md) | **SIM-DEMO-1 / SIM-DEMO-2** — dashboard + tên file gợi ý; chi tiết ticket trong `DEMO-PROJECT-BRIEF.md` |
| [`docker-compose.postgres.yml`](docker-compose.postgres.yml) | PostgreSQL 16 (xem brief §6) |

## Gợi ý thứ tự demo

1. Đọc [`../../al-run/task-lifecycle/README.md`](../../al-run/task-lifecycle/README.md) → [`FROM-TICKET-TO-DONE.md`](../../al-run/task-lifecycle/FROM-TICKET-TO-DONE.md).
2. Đọc `DEMO-PROJECT-BRIEF.md`, rồi **`.operational-resources/docs/README.md`** và các file ShelfLog trong `docs/` (brief §9).
3. Làm **SIM-DEMO-1** trước (infra), rồi **SIM-DEMO-2** (API); cập nhật dashboard `al-run/current-task/README.md`.
4. Mỗi ticket: [`../skills/workflow/implement-feature/`](../skills/workflow/implement-feature/) hoặc skill ops tương ứng; đổi contract → chỉ **`.operational-resources/docs/`** (template sync).
