...
> **Bản sao cá nhân trong repo này:** xem thêm [`../README.md`](../README.md) — tách **`al-run/`** (task + gate + lifecycle) và **`workspace/`** (file này, `AGENTS.md`, `docs/`, `rules/`, `skills/`, …). Sơ đồ dưới là **cây template** `.operational-resources/` (sync team).
...
├── simulator/                # Demo A→Z: brief + Compose Postgres (task canonical ở current-task/)
│   ├── README.md
│   ├── DEMO-PROJECT-BRIEF.md
│   └── docker-compose.postgres.yml
├── README-GIT-LOCAL.md       # Ẩn/hiện thư mục này khỏi Git local
├── RESOURCE-CHANGE-LOG.md    # Nhật ký delta tài nguyện (rules/skills/docs/…)
...
| **`RESOURCE-CHANGE-LOG.md`** | Nhật ký **delta** tài nguyện (`rules/`, `skills/`, `docs/`, …) — gốc bundle; bản `use1000`: `.operational-resources-use1000/workspace/RESOURCE-CHANGE-LOG.md`. |
...
| File | Mục đích |
|------|----------|
| **`simulator/README.md`** | Cách dùng: brief + luồng `task-lifecycle/` + trỏ task canonical trong `current-task/`. |
...
| Nội dung | Mục đích |
|----------|----------|
| **`00-personal-priority.md`** | Ưu tiên cao nhất; đọc trước; trỏ task tập trung tại `current-task/`. |
| **`01`–`08-*.md`** | Tổng quan dự án, coding standards, API, frontend/backend, security, testing, review checklist. |
| **`README.md`** | Giải thích: **không** đặt file task trong `rules/`; task chỉ ở `current-task/`. |

**Lưu ý:** Không chứa file task `current-task-*.md` — đã loại bỏ để tránh trùng với `current-task/`.
...
5. `current-task/<file>.md` (task đang làm)  
6. `skills/...` theo bảng trong task file  
7. `docs/specs/README.md`, `docs/api/`, `knowledge-base/` khi cần  
8. Demo ShelfLog: `simulator/DEMO-PROJECT-BRIEF.md` → `docs/project-overview.md` → `docs/specs/feature-shelflog-items.md` → `docs/api/08-endpoint-list.md`  
...
**Last updated:** 2026-04-17 (`RESOURCE-CHANGE-LOG.md` ở gốc bundle; ShelfLog: một nguồn `docs/`; không còn `simulator/docs/`)
