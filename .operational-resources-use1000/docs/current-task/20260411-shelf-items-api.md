...
- Luá»“ng: **`task-lifecycle/`**. Task canonical: `docs/current-task/20260411-shelf-items-api.md`.
...
- **Task canonical:** `.operational-resources/docs/current-task/20260411-shelf-items-api.md`.
...
- [x] **AC1:** `POST /api/v1/shelf-items` táº¡o tÃ i nguyÃªn; tráº£ **201** + body cÃ³ `id`, timestamp vÃ  cÃ¡c trÆ°á»ng Ä‘Ã£ lÆ°u.
- [x] **AC2:** `GET /api/v1/shelf-items/{id}` tráº£ **200** khi tá»“n táº¡i, **404** khi khÃ´ng cÃ³.
- [x] **AC3:** `PUT /api/v1/shelf-items/{id}` thay tháº¿ trÆ°á»ng Ä‘Æ°á»£c phÃ©p; **404** khi khÃ´ng cÃ³; validation nhÆ° lÃºc táº¡o.
- [x] **AC4:** `DELETE /api/v1/shelf-items/{id}` tráº£ **204** vÃ  xÃ³a báº£n ghi; **404** khi khÃ´ng cÃ³.
- [x] **AC5:** `GET /api/v1/shelf-items` há»— trá»£ `page`, `size` (tá»‘i Ä‘a 100), lá»c tuá»³ chá»n `category`; JSON phÃ¢n trang **á»•n Ä‘á»‹nh** (Ä‘Ã£ ghi trong doc).
- [x] **AC6:** `quantity` sai (>999 hoáº·c Ã¢m), `title` rá»—ng, `category` khÃ´ng há»£p lá»‡ â†’ **400** vá»›i payload lá»—i nháº­n diá»‡n Ä‘Æ°á»£c.
...
|----------------|-------|--------------------------------|------|
| AC1 | Táº¡o há»£p lá»‡ | `ShelfItemControllerTest#create_returns201` | integration |
| AC2 | GET theo id | `ShelfItemControllerTest#get_found_and_notFound` | integration |
| AC3 | PUT cáº­p nháº­t | `ShelfItemControllerTest#put_updates_fields` | integration |
| AC4 | DELETE | `ShelfItemControllerTest#delete_then_get_404` | integration |
| AC5 | List + lá»c | `ShelfItemControllerTest#list_pagination_and_category` | integration |
| AC6 | Validation | `ShelfItemControllerTest#validation_errors` | integration |
...
## 8. Context pack cho AI - rules, docs, skills
...
| Docs | `docs/current-task/20260414-shelflog-infra.md`, `docs/specs/feature-shelflog-items.md`, `docs/api/08-endpoint-list.md`, `docs/api/10-current-api-changes.md`, `docs/setup/02-local-development.md`, `docs/decisions/006-shelflog-demo-postgres-docker.md` |
...
Prompt goi y (copy):
"Doc `docs/current-task/20260414-shelflog-infra.md` (da Done), sau do doc `20260411-shelf-items-api.md`, `docs/specs/feature-shelflog-items.md`, `docs/setup/02-local-development.md`, `simulator/DEMO-PROJECT-BRIEF.md`. Chi implement API + domain ShelfItem; khong lap lai baseline infra SIM-DEMO-1. Cap nhat docs khi doi contract."

---

## 9. Checklist thá»±c thi (tÃ¹y chá»‰nh)

- [x] RÃ ng buá»™c brief vÃ o entity/DTO (Ä‘á»™ dÃ i title, quantity 0â€“999, enum).
- [x] Controller dÆ°á»›i `/api/v1/shelf-items`; body lá»—i thá»‘ng nháº¥t.
- [x] Repository + `Pageable` cho list.
- [x] Ãt nháº¥t má»™t class integration test cho AC1â€“AC6 (profile test).
- [x] `./mvnw test` xanh.
- [x] Doc (`.operational-resources/docs/`): `api/10-current-api-changes.md` Ä‘Ã£ cáº­p nháº­t.

---

## 10. HÆ°á»›ng dáº«n chi tiáº¿t cho AI (báº¯t buá»™c â€” Ã­t nháº¥t ba Ã½)

- **MUST:** Validate `quantity` trong [0..999] va `title` trong [1..200] tai boundary API; khong lap lai baseline infra SIM-DEMO-1.
- **SHOULD:** Dat default `page/size` hop ly, gioi han `size <= 100`, va co IT cho validation + pagination/filter.
- **ASK FIRST:** Bat ky starter Maven moi, Testcontainers, hoac thay doi global exception/shared interface.
...
- **[2026-04-16]** Implement ShelfItem slice: entity/repository/service/controller, global exception handler, va `ShelfItemControllerTest` cover AC1-AC6.
- **[2026-04-16]** Cháº¡y `./mvnw.cmd -q test` pass; evidence: `docs/current-task/logs/20260416-shelf-items-test-evidence.txt`.
...
**Cáº­p nháº­t láº§n cuá»‘i:** 2026-04-16 (SIM-DEMO-2 implemented)
