...
- **SIM-DEMO-1** — Chuẩn bị **hạ tầng thật** cho demo ShelfLog: dependencies Spring Web / Validation / JPA / Actuator, Postgres (`dev`) + H2 (`test`), cấu hình YAML, IT kiểm tra `/actuator/health`.
- **Phải xong trước** ticket **SIM-DEMO-2** (`20260411-shelf-items-api.md` — API shelf items).
- Tài liệu: chỉ **`.operational-resources/docs/`**; ticket mẫu: `simulator/DEMO-TICKET-20260414-shelflog-infra.md`.
